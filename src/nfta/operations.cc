...
const utils::OrdVector<SymbolArity>& resolve_symbols_arities(
    ...
    if (symbols_arities_in) {
        return *symbols_arities_in;
    }
...
Nfta remove_epsilon(const Nfta& aut, const Symbol epsilon) {
    ...
    for (State i = 0; i < num_of_states; i++) {
        for (const State closure_of_i : epsilon_closures[i]) {
            if (aut.is_state_root(closure_of_i)) {
                result.add_root(i);
            }
...
void Nfta::unite_nondet_with(const Nfta& aut) {
    ...
    auto renumber_states = [&](const State st) { return static_cast<State>(st + orig_num_of_states); };
...
Nfta union_nondet(const Nfta& A, const Nfta& B) {
    if (A.root_states.empty() && B.root_states.empty()) {
        return create_empty(A.alphabet);
    }
    Nfta result{A};
...
struct ProductContext {
    ...
    explicit ProductContext(
            const size_t num_states_A, const size_t num_states_B, utils::TwoDimensionalMap<State>* state_mapping_out)
...
Nfta union_det_on_complete(const Nfta& A, const Nfta& B, utils::TwoDimensionalMap<State>* state_mapping_out) {
    ...
    auto fill_cache = [&](std::unordered_map<Symbol, SymbolCache>& cache, const ReversedDelta& rev_delta,
    ...
    auto try_build_source_tuple = [&](const ReversedDelta::RevStateTuplePost& src_tr_A,
                                      const ReversedDelta::RevStateTuplePost& src_tr_B, unsigned arity,
                                      std::vector<State>& source_tuple) -> bool {
        assert(src_tr_B.sources.size() == arity);
        for (size_t i = 0; i < arity; ++i) {
            const State ps = ctx.state_mapping->get(src_tr_A.sources[i], src_tr_B.sources[i]);
            if (ps == Limits::max_state) {
                return false;
            }
            source_tuple[i] = ps;
        }
    ...
    auto process_target_pair = [&](const Symbol symbol, const std::vector<State>& source_tuple, const State target_A,
                                   const State target_B) {
            ...
            for (unsigned i = 0; i < arity; ++i) {
                for (auto src_tr_A : state_A_cache[i]) {
                    for (auto src_tr_B : state_B_cache[i]) {
                        if (!try_build_source_tuple(*src_tr_A, *src_tr_B, arity, source_tuple))
                            continue;
    ...
    for (const auto& state_post : delta) {
        for (const auto& symbol_post : state_post) {
            for (const auto& targets : symbol_post.target_tuples) {
                ...
                if (std::ranges::find(moves, current) != moves.end()) {
                    return false;
                }
...
bool Nfta::is_top_down_deterministic() const {
    ...
    for (State i = 0; i < num_of_states; ++i) {
        for (const auto& symbol_post : delta[i]) {
            if (symbol_post.target_tuples.size() != 1) {
                return false;
            }
        }
    }
...
bool Nfta::is_complete(const utils::OrdVector<Symbol>& symbols) const {
    ...
    for (const auto& symbol_tr : rev_delta.symbol_posts) {
        ...
        if (symbol_tr.state_tuple_posts.size() != ipow(num_of_states, arity)) {
            return false;
        }
...
void Nfta::make_complete(const utils::OrdVector<SymbolArity>* symbols_arities_in, State sink) {
    ...
    if (sink == Limits::max_state) {
        sink = delta.add_state();
    }
    ...
    while (rev_delta_it != rev_delta_end || input_symbols_it != input_symbols_end) {
        ...
        if (rev_delta_it != rev_delta_end &&
            (input_symbols_it == input_symbols_end || rev_delta_it->symbol < input_symbols_it->first)) {
            if (input_symbols_it == input_symbols_end) {
                std::cerr << "Input symbols end" << std::endl;
            }
        ...
        if (delta_empty || rev_delta_it == rev_delta_end || symbol < rev_delta_it->symbol) {
            ...
            if (old_rev_delta_it->state_tuple_posts.size() == expected_num_of_transitions) {
                continue;
            }
        } // else
        ...
        if (sink_sp_empty) {
            ...
            delta.add(sink, new_symbol_post);
        }
...
BoolVector Nfta::get_bottom_up_reachable_impl(OnMarked&& early_exit_fn, const BoolVector* allowed) const {
    using SymbolCache = std::vector<std::vector<utils::OrdVector<const ReversedDelta::RevStateTuplePost*>>>;
            ...
            for (unsigned pos = 0; pos < symbol_cache[current].size(); ++pos) { // todo cach by pos has no benefit
                for (const auto* src_tr : symbol_cache[current][pos]) {
                    ...
                    for (const State target : src_tr->targets) {
                        if (!marked[target]) {
                            mark(target);
                            if (early_exit_fn(target, marked)) {
                                return marked;
                            }
                        }
                    }
...
bool Nfta::is_lang_empty() const {
    if (root_states.empty() || delta.empty())
        return true;
    const BoolVector reachable =
            get_bottom_up_reachable_impl([&](const State s, const BoolVector&) { return root_states.contains(s); });
...
bool is_lang_included(const Nfta& smaller, const Nfta& bigger, const ComplementMethod method) {
    ...
    if (method == ComplementMethod::Classical) {
        ...
        throw std::runtime_error("Complement method not implemented");
    }
