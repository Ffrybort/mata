 ...
 */
RawTransition
get_transition(const FormulaNode& formula_node, const FormulaGraph& formula_graph, const NameStateMap& state_map) {
    RawTransition transition;
    transition.source = state_map.at(formula_node.name);

    transition.symbol_string =
            formula_graph.node.is_and() ? formula_graph.children[0].node.name : formula_graph.node.name;
...
Nfta construct_from_inter_aut(const IntermediateAut* inter_aut, RankedAlphabet* alphabet) {
    ...
    if (alphabet == nullptr) {
        alphabet = new RankedOnTheFlyAlphabet();
    }
    Nfta aut;
    ...
    // symbols (and arities)
    // symbols are added for nfta even if they are not enumerated, they are checked for validity so why not also add
    // them
...
Nfta construct_from_inter_aut(const IntermediateAut* inter_aut, Alphabet* alphabet) {
    ...
    for (const auto& symbols_name : inter_aut->symbols_names) {
        alphabet->try_add_new_symbol(symbols_name);
    }
...
Nfta parse_from_mata(std::istream& input, Alphabet* alphabet) {
    ...
    if (ia.overload) {
        throw std::runtime_error("Symbol overload requires a ranked alphabet.");
    }
    return construct_from_inter_aut(&ia, alphabet);
} // parse_from_mata
